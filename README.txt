...
------------
		...
		4) Go to the URL for your EvalCOMIX Server in a browser to start the installation wizard
...
- Source code of EvalCOMIX Server
- Database.
...
2) Unzip or unpack the upgrade file so that all the new EvalCOMIX Server software program files 
   are in the location the old files used to be in on the server.
   
3) Go to the URL for your EvalCOMIX Server in a browser. The wizard will ask you for the
    database connection data. They should be the same as the previous version of EvalCOMIX Server.
    EvalCOMIX Server will tune the database if the upgrade requires it. It will also ask you
    creating a new user.
...
4) Once the update is complete, access the control panel with the new user's credentials
    ...
    this instance of EvalCOMIX Server.	
...
- Código fuente de EvalCOMIX Server
- Base de datos.
...
2) Descomprima el archivo de instalación, de forma que todos los archivos de EvalCOMIX Server
   estén ahora en el mismo sitio donde ANTES estaban los archivos de la versión anterior de EvalCOMIX Server.
   
3) Acceda a través del navegador a la URL de EvalCOMIX Server. El asistente le solicitará los 
   datos de conexión de la base de datos. Deberán ser los mismos que los de la versión anterior de EvalCOMIX Server.
   EvalCOMIX Server ajustará la base de datos si así lo requiere la actualización. También le solicitará
...
4) Una vez completada la actualización, acceda al panel de control con las credenciales del nuevo usuario
   ...
   esta instancia de EvalCOMIX Server.
