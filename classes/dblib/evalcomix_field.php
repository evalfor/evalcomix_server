...
require_once('evalcomix_dbobject.php');
...
class evalcomix_field extends evalcomix_dbobject{
    ...
    /** @var array defines valid types */
  	private $valid_types = array(EVALCOMIX_TYPE_INTEGER, EVALCOMIX_TYPE_NUMERIC, EVALCOMIX_TYPE_FLOAT,
							EVALCOMIX_TYPE_CHAR, EVALCOMIX_TYPE_TEXT, EVALCOMIX_TYPE_BINARY, EVALCOMIX_TYPE_DATETIME,
							EVALCOMIX_TYPE_TIMESTAMP, EVALCOMIX_TYPE_BOOL, EVALCOMIX_TYPE_BIGINT,
							EVALCOMIX_TYPE_SMALLINT);
    /**
     * Note:
    ...
    /// each one as needed.
		...
		if(isset($params['precision'])){
			// This is necessary because Postgres does not support precision on integer data types.
			$precisionarr = explode(',', $params['precision']);
			if ($this->type != EVALCOMIX_TYPE_INTEGER && $this->type != EVALCOMIX_TYPE_SMALLINT
				&& $this->type != EVALCOMIX_TYPE_BIGINT) {
				...
				// Smallint is standard data type in all databases.
				$precision = trim($precisionarr[0]);
				if ((int)$precision > 0 && (int)$precision <= 2) {
					$this->type = EVALCOMIX_TYPE_SMALLINT;
				}
			}
        }
		$notnull = null;
