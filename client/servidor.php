<?php
	...
	if(isset($print)){
		...
		/*$tool->display_view($postCleaned, "print");
		$tool->set_view('view',$id);
		$tool->display_body_view($postCleaned);
		$tool->display_footer();
...
//Si estamos trabajando con instrumentos completos (mixtos o simples)
		...
		if(isset($postCleaned['save']) && (isset($postCleaned['addtool'.$id]) || isset($postCleaned['addDim']) || isset($postCleaned['addtool']))){
			...
			//$plataforma = $_SESSION['plataforma'];//echo "curso:$curso -- plataforma:$plataforma";
			//$get;
			if(isset($id))
				$get = '&id='.$id;
			...
			if($error == false){
				$xml = $tool->export();//echo htmlspecialchars($xml);exit;
				include_once('../configuration/host.php');
				include_once('../classes/curl.class.php');
				$url = HOST . 'webservice/import_tool.php?id='.$id;//.$get;
					...
					if(isset($xml2->status) && (string)$xml2->status == 'success' && isset($xml2->description)){
						...
						if($tool->type == 'mixta'){
							...
							switch($tool->type){
								...
								case 'listaescala':{
									$tool_valoreslista_id = $tool->valoreslista;
									$tool_valoreslista = current($tool_valoreslista_id);
								}break;
								case 'diferencial':{
							...
							if(isset($tool_aux_dimensionsId) && isset($tool_aux_subdimensionsId) && isset($tool_aux_atributosId)){
								foreach($tool_dimensions as $dim => $value_dimensions){
									...
									foreach($tool_valores[$dim] as $grado => $values){
										if(list($keyv, $datav) = each($tool_aux_valoresId[$key2])){
								...
								//$res = $tool->get_dimensionsid();
								//$res2 = $tool->get_subdimensionsid();print_r($res2);
							}
							if(isset($tool_aux_valorestotalesId)){
						...
						echo "<script type='text/javascript'>alert('Error en la respuesta del servidor');</script>";//echo htmlentities($response);
					}
				}
				else{
...
//		elseif(isset($postCleaned['addtool'.$id]) || isset($postCleaned['addDim']) || isset($postCleaned['addtool'])){
