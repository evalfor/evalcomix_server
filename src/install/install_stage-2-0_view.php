...
echo '
<input type="hidden" name="substage" value="4">
';

foreach ($result as $tablename => $status) {
	$bgcolor = 'bg-success text-white p-5';
	$message = get_string('tablecreated');
	if ($status === false) {
		$bgcolor = 'bg-danger text-white p-5';
		$message = get_string('failedcreatetable');
	} else if ($status === 0) {
		$bgcolor = 'bg-warning text-white p-5';
		$message = get_string('tablealreadyexists');
	}
	echo '<h3 class="'.$bgcolor.'">'.strtoupper($tablename). ' ' . $message . '</h3>';
}
